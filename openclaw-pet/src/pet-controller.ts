...
export function createPetController(config: PetConfig = {}) {
  ...
  return {
    ...
    toolFinished: (failed: boolean, displayLabel?: string) => { activityCount += 1; const label = displayLabel ?? (failed ? "Tool failed" : "Tool complete"); set(failed ? "failed" : "waiting", failed ? "tool-failed" : "tool-finished", label); record(label, failed ? "error" : "success"); if (activeRuns > 0) showActiveRun(); else if (!failed) scheduleIdle(); },
    agentEnded: (failed: boolean, displayLabel?: string) => { activityCount += 1; activeRuns = Math.max(0, activeRuns - 1); const label = displayLabel ?? (failed ? "Task failed" : "Task complete"); record(label, failed ? "error" : "success"); if (activeRuns > 0) { showActiveRun(); return; } set(failed ? "failed" : "jumping", failed ? "agent-failed" : "agent-finished", label); scheduleIdle(); },
  };
}
