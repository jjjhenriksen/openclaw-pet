...
The plugin reduces OpenClaw lifecycle events to display-only pet state in the platform-neutral controller. Its versioned Gateway bridge exposes only an allowlisted `{ animation, changedAt, activityLabel, activity, runs }` snapshot. Each bounded recent run has an opaque ID, sanitized session kind/display name/agent ID, lifecycle state, validated tool name, timestamps, and attention/unread flags. Raw prompts, tool arguments/results, session keys, run IDs, credentials, and arbitrary event text never enter the bridge. Activity labels are generated from fixed lifecycle phases and validated tool names; prompts, tool arguments, tool results, model output, arbitrary event titles/statuses, credentials, asset paths, and controller errors never enter a bridge snapshot. Session context is extensible by kind: cron runs show only the configured cron display name when available and the validated agent ID (`Cron "name" · agent id`); other sessions use a persisted session display name when available (`Session "name" · agent id`) and otherwise the generic `Session · agent id` shape. The plugin never reads transcript text for display. Prompts, delivery targets, raw session keys, job identifiers, and run identifiers are never displayed. Labels are trimmed, control characters are removed, and bounded to 80 characters. Do not put secrets in a session or cron name.
